...
async function createMediumWidget() {
    ...
    oWidget.setPadding(10, 15, 10, 10)
    if (WIDGET_URL.length > 0) {
        oWidget.url = WIDGET_URL;
    }
...
async function addSmallWidgetData(oWidget) {
    ...
    // oGameData.setPadding(10, 15, 10, 10)
...
async function addMediumWidgetData(oWidget) {
    ...
    if (oGameData != null) {
        ...
        let oHeadingText;
        if (
            oGameData.gameStatus != undefined && oGameData.gameStatus != "" && oGameData.homeTeam.liveScore != "-" && SHOW_LIVE_SCORES) {
            // alert(`${oGameData.gameStatus}`)
            // let myAlert = new Alert();
            // myAlert.addTextField(`${oGameData.gameStatus}`)
            ...
            oHeadingText = oHeadingStack.addText(`${dLocalDate}  |  主队 : ${oGameData.simpleName}  | 📍: ${oGameData.venue}`);
        }
        oHeadingText.font = Font.boldSystemFont(11);
        oHeadingText.textColor = getColorForCurrentAppearance();
        ...
        oFutureGamesStack.setPadding(2, 0, 2, 0);
        oFutureGamesStack.addSpacer();
        oFutureGamesStack.size = new Size(340, 20);
        ...
        for (let i = 0; i < oGameData.nextGames.length; i++) {
            ...
            // const oTeamData = getTeamData();
            const oFutureGameLogoImage = await loadLogo(oNextGame.opponent.logoLink,oNextGame.opponent.abbreviation);
            const oNextGameLogo = oFutureGame.addImage(oFutureGameLogoImage,"\r\n");
            oNextGameLogo.imageSize = new Size(15, 14);
            ...
            // oTeamData.abbreviation

            const oNextGameText = oFutureGame.addText(`${oNextGame.opponent.abbreviation}${dLocalDate}`);
            oNextGameText.font = Font.systemFont(10);
        ...
        const oHeadingText = oHeadingStack.addText(`No upcoming games. Season ended.`);
        oHeadingText.font = Font.boldSystemFont(11);
        oHeadingText.textColor = getColorForCurrentAppearance();
...
async function addHomeTeamStack(oNextGameStack, oGameData) {
    ...
    const oHomeLogoImage = await loadLogo(oGameData.homeTeam.logoLink,oGameData.homeTeam.abbreviation);
    ...
    if (SHOW_LIVE_SCORES) {
        ...
        const iHomeTeamLiveScoreSpacer = iHomeTeamLiveScore < 99 || iHomeTeamLiveScore === "-" ? 45 : 25;
        oHomeTeamLogoStack.addSpacer(iHomeTeamLiveScoreSpacer);
        const oHomeTeamGoalsText = oHomeTeamLogoStack.addText(`${iHomeTeamLiveScore}`);
    ...
    if (SHOW_STATS_AND_STANDINGS) {
        ...
        oHomeTeamStatsText.font = Font.systemFont(13);
        ...
        if (oGameData.homeTeam.topscorer.name != null) {
            const oHomeTeamTopScorerText = oHomeTeamStack.addText(`${oGameData.homeTeam.topscorer.name} (${oGameData.homeTeam.topscorer.value})`);
...
async function addAwayTeamStack(oNextGameStack, oGameData) {
    ...
    const oAwayLogoImage = await loadLogo(oGameData.awayTeam.logoLink,oGameData.awayTeam.abbreviation);
    ...
    if (SHOW_LIVE_SCORES) {
        ...
        const iSpacer = iAwayTeamLiveScore < 99 || iAwayTeamLiveScore === "-" ? 45 : 25;
        oAwayTeamLogoStack.addSpacer(iSpacer);

        const oAwayTeamGoalsText = oAwayTeamLogoStack.addText(`${iAwayTeamLiveScore}`);
    ...
    if (SHOW_STATS_AND_STANDINGS) {
        ...
        oAwayTeamStatsText.font = Font.systemFont(13);
...
async function prepareData() {
    ...
    if (aScheduleData && aScheduleData.length > 0) {
        ...
        if (oNextGame != undefined) {
            ...
            oData.simpleName = oHomeTeam.simpleName;
